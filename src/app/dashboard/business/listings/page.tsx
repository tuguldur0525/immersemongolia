...
import {
  Plus, Search, Filter, Eye, Edit3, Trash2, MoreVertical,
  TrendingUp, Star, MapPin, CheckCircle, Clock, XCircle,
  Building2, BarChart2, CreditCard, Users, Settings, LogOut, Menu, X
} from 'lucide-react'
...
const SAMPLE_LISTINGS = [
  {
    id: '1', name: 'Монголын Элч Ресторан', status: 'ACTIVE',
    category: 'Ресторан', views: 1840, saves: 142, rating: 4.3, reviews: 67,
    isVerified: true, isFeatured: true, plan: 'PROFESSIONAL',
    coverImageUrl: null, slug: 'mongolian-ambassador',
  },
  {
    id: '2', name: 'Гандан Кафе', status: 'PENDING_REVIEW',
    category: 'Кафе', views: 320, saves: 28, rating: 0, reviews: 0,
    isVerified: false, isFeatured: false, plan: 'STARTER',
    coverImageUrl: null, slug: 'gandan-cafe',
  },
]
...
export default function BusinessListingsPage() {
  ...
  const filtered = SAMPLE_LISTINGS.filter(b =>
      ...
      <>
        ...
        <aside className={cn(
          ...
          <div className="p-3 border-t border-border">
            <div className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-background-secondary cursor-pointer transition-colors">
              <div className="size-8 rounded-full bg-brand-primary/15 flex items-center justify-center text-xs font-bold text-brand-primary">М</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">Мөнхбаяр Б.</p>
                <p className="text-xs text-foreground-muted">Бизнес эзэн</p>
              </div>
            </div>
            <button className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-foreground-muted hover:text-brand-danger hover:bg-brand-danger/8 transition-colors w-full mt-1">
              <LogOut size={16} />
              Гарах
            </button>
          </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="h-16 bg-card border-b border-border flex items-center gap-4 px-4 sm:px-6 sticky top-0 z-20">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-foreground-muted hidden sm:block">Бүртгэлтэй бизнесүүдийн удирдлага</p>
          </div>
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Нийт харалт', value: '2,160', delta: '+12%', icon: Eye, color: 'text-brand-primary', bg: 'bg-brand-primary/10' },
              { label: 'Хадгалсан', value: '170', delta: '+8%', icon: TrendingUp, color: 'text-brand-success', bg: 'bg-brand-success/10' },
              { label: 'Үнэлгээ', value: '4.3', delta: '+0.2', icon: Star, color: 'text-brand-accent', bg: 'bg-brand-accent/10' },
              { label: 'Санал хүсэлт', value: '67', delta: '+5', icon: Users, color: 'text-brand-secondary', bg: 'bg-brand-secondary/10' },
            ].map(stat => (
              <motion.div
                key={stat.label}
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs font-medium text-brand-success">{stat.delta}</span>
                </div>
          ...
          <div className="bg-card rounded-2xl border border-border overflow-hidden">
            ...
            <div className="p-4 border-b border-border flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
              <h2 className="font-semibold">Бүртгэлтэй бизнесүүд ({SAMPLE_LISTINGS.length})</h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map((biz, i) => {
                    const statusConfig = STATUS_CONFIG[biz.status as keyof typeof STATUS_CONFIG]
                      ...
                      >
                        ...
                        <td className="px-4 py-4 hidden md:table-cell">
                          <div className="flex items-center gap-1.5 text-sm">
                            ...
                            <span>{formatNumber(biz.views)}</span>
                          </div>
                        </td>
                        <td className="px-4 py-4 hidden lg:table-cell">
                          {biz.rating > 0 ? (
                            <div className="flex items-center gap-1.5 text-sm">
                              ...
                              <span className="text-foreground-muted">({biz.reviews})</span>
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-4 hidden lg:table-cell">
                          <span className={cn('px-2.5 py-1 rounded-lg text-xs font-medium',
                            biz.plan === 'PROFESSIONAL' ? 'bg-brand-primary/10 text-brand-primary' : 'bg-background-tertiary text-foreground-muted'
                          )}>
                            {biz.plan}
                          </span>
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-1 justify-end">
                            ...
                            <Link href={`/dashboard/business/listings/${biz.id}/edit`}
                              className="size-8 rounded-lg hover:bg-background-tertiary flex items-center justify-center text-foreground-muted hover:text-foreground transition-colors">
                              <Edit3 size={15} />
                            </Link>
                            <div className="relative">
                              ...
                              {openMenu === biz.id && (
                                <div className="absolute right-0 top-full mt-1 w-40 rounded-xl bg-card border border-border shadow-lg py-1 z-10">
                                  <button className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground-secondary hover:bg-background-secondary transition-colors">
                                    <TrendingUp size={14} />
                                    Аналитик
                                  </button>
                                  <button className="w-full flex items-center gap-2 px-3 py-2 text-sm text-brand-danger hover:bg-brand-danger/8 transition-colors">
                                    <Trash2 size={14} />
                                    Устгах
                                  </button>
                                </div>
              ...
              {filtered.length === 0 && (
                <div className="py-20 text-center">
                  ...
                  <p className="font-medium mb-1">Бизнес олдсонгүй</p>
